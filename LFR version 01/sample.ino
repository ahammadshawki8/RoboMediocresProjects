...
void loop() {
  ...
  int s1 = 1 - digitalRead(ir1); // Leftmost sensor
  int s2 = 1 - digitalRead(ir2); // Left sensor
  int s3 = 1 - digitalRead(ir3); // Middle sensor
  int s4 = 1 - digitalRead(ir4); // Right sensor
  int s5 = 1 - digitalRead(ir5); // Rightmost sensor
  ...
  if ((s1 == 1) && (s2 == 1) && (s3 == 0) && (s4 == 1) && (s5 == 1)) {
    ...
    analogWrite(e1, 255); // Set right motor speed to full
    analogWrite(e2, 255); // Set left motor speed to full
  ...
  if ((s1 == 1) && (s2 == 0) && (s3 == 1) && (s4 == 1) && (s5 == 1)) {
    analogWrite(e1, 255); // Set motor speeds
    analogWrite(e2, 255);
    digitalWrite(m1, HIGH); // Right motor forward
    digitalWrite(m2, LOW);
